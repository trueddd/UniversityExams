...
•	Система класса CP в каждый момент обеспечивает целостный результат и способна функционировать в условиях распада, но достигает этого в ущерб доступности: может не выдавать отклик на запрос. Устойчивость к распаду на секции требует обеспечения дублирования изменений во всех узлах системы.
...
Вертикальное масштабирование предполагает добавление большего количества ресурсов к отдельному серверу (Например добавление большего количества жестких дисков, чтобы весь объем данных разместился на одном узле)
Горизонтальное масштабирование предполагает добавления большего количества узлов (Например для большого объема данных будет добавлен новый узел, для хранения части данных)
