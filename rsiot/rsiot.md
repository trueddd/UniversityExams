...
Предохранительный уровень - это отношение связывания контекстов, обеспечивающее их защиту. В этом случае нижестоящая группа создает уровень перевода между своим **единым языком** (моделью) и **единым языком** (моделью) вышестоящей группы. Этот уровень изолирует нижестоящую модель от вышестоящей и обеспечивает перевод сообщений между ними. Таким образом, это отношение также является разновидностью интеграции.
...
Один из наиболее популярных способов использования RPC - простой протокол доступа к объектам (Simple Object Access Protocol- SOAP)). Идея, лежащая в основе использования механизма RPC с протоколом SOAP, заключается в том, чтобы сделать использование службы из другой системы похожей на вызов простой локальной процедуры или вызов метода. И все же запрос SOAP должен пройти по сети, достигнуть отдаленной системы, успешно выполниться и вернуть результаты по сети. В итоге возникает возможность полного отказа сети или, по краинеи мере, непредвиденной задержки при первой попытке интеграции.
Основная проблема. связанная с использованием механизма RPC на основе протокола SOAP заключается в том, что этот механизм может оказаться ненадежным.

Интеграция с помощью протокола RESТful НТТР сосредоточивает внимание на ресурсах, которыми обмениваются **ограниченные контексты**, а также на четырех элементарных операциях: POST, GET, РUТ и DELETE. Многие считают, что подход REST хорош для интеграции, потому что он помогает определять хорошие интерфейсы АРI для распределенных вычислений. Трудно возражать против этого утверждения, учитывая успех Интернета и веб.

Рассылка сообщений - одна из наиболее устойчивых форм интеграции, потому что с ее помощью удаляется большая часть временных связей, возникающих из-за таких механизмов блокировки, как RPC и RESТ.

Проблема - **Большой ком грязи**

Если вы все же создали **Большой ком грязи**:
1. возрастающее количество **агрегатов** загрязняет друг друга из-за незащищенных связей и зависимостей; 
...
Поколько монолитная версия хранит свои данные в одной БД, она может легко извлечь подробности о заказе с помощью выражения SELECT, которая объединяет таблицы.Тогда как в микросервисном приложении  информация разбросана по сервисам и поэтому любой клиент, которому нужны подробности о заказе, должен опросить все сервисы.
Для таких запросов можно использовать шаблон **Объединение API**.

Проблема в том,что если нужно выполнить один запрос,( как на картинке "найти заказ"), то нужно делать запрос на все сервисы. которые связанны с изначальным запросом

Шаблон **Объединение API** реализует запрос, который извлекает данные из разных сервисов, обращаясь к ним через их API и объединяя результат. 
...
* Нехватка транзакционной соглассованности данных

#### 55. CQRS. Запрашивающий сервис.

**СQRS(Command Query Responsibility Segregation)** - Шаблон проектирования основанные на разделении ответственности командных запросов.Командные модули и модель данных реализуют операции которые соответствуют HTTP запросам.

![img1](./55/55_1.png)

В сервисе основанном на CQRS, доменная модель командной стороны обрабатывает CRUD-операции и привязана с собственной БД. За нетривиальные запросы отвечает отдельная модель, которая проще командной стороны т.к. она не реализует бизнес-логику.Для поддержки запросов эта модель использует подходящую для этого БД.
...
* Отставание репликации - когда клиент, который обновляет командную строку и затем немедленно выполняет запрос, может не увидеть собственное обновление.
...
DevOps(development and operations) — методология задача которой сократить время разработки программного обеспечения и ускорить выпуск обновлений и патчей к нему. 
...
В данный момент очень популярен принцип CI/CD разработки. В нем DevOps инженеры играют важную роль. 
![img1](./57/57_1.png)
В данном подходе к разработке DevOps инженеры есут ответственность за доступность продукта пользователям. Работая в области CI/CD, они масштабируют концепции по мере необходимости и разрабатывают логистику кода, чтобы код от разработчиков мог перейти в производственную среду и пользователи могли получать доступ. В их обязанности входить настройка билдов, тестов, релизов, деплоев, мониторинга. В данный момент времени для DevOps инженеров важны знанией такие технологий как Nginx, K8S, Docker, AWS, GCP, ElasticSearch, RabbitMQ и др.
