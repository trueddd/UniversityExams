...
Поколько монолитная версия FTGO хранит свои данные в одной БД, она может легко извлечь подробности о заказе с помощью выражения SELECT, которая объединяет таблицы.Тогда как в микросервисном приложении FTGO информация разбросана по сервисам и поэтому любой клиент, которому нужны подробности о заказе, должен опросить все сервисы.
...
**СQRS** - Разделение ответственности командных запросов.Командные модули и модель данных реализуют операции которые соответствуют HTTP запросам.
